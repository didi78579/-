...
public class BeaconBlockEventHandler implements Listener {
    ...
    private void handleBeaconPlacement(BlockPlaceEvent event, Player player, Block block) {
        ...
        if (!isValidBeaconPlacement(block)) {
            event.setCancelled(true);
            player.sendMessage(ChatColor.RED + "신호기의 설치 위치가 올바르지 않습니다.");
            return;
        }
    ...
    private boolean isValidBeaconPlacement(Block beaconBlock) {
        int seaLevel = beaconBlock.getWorld().getSeaLevel();

        // 신호기가 바다 레벨 아래에 있으면 설치 불가
        if (beaconBlock.getY() > seaLevel) {
            return false;
        }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        // 블록이 신호기 또는 신호기 기반(철 블럭)이 아닌 경우 종료
        if (block.getType() != Material.BEACON && !isPartOfBeaconBase(block)) {
            return;
        }
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        long banDuration = 3600000L; // 1시간 밴(밀리초)
        playerReviveManager.banPlayer(player.getUniqueId(), banDuration);

        new BukkitRunnable() {
            @Override
            public void run() {
                player.kickPlayer(ChatColor.RED + "사망하여 1시간 동안 밴되었습니다.");
            }
